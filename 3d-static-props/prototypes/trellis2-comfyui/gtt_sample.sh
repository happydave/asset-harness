...
# GTT used, carve-out used, MemAvailable, GPU busy, and the RSS of one container's main process.
...
mib() { echo $(( $1 / 1048576 )); }
echo -e "ts\tgtt_used_mib\tvram_used_mib\tmem_available_mib\tgpu_busy_pct\tctr_rss_mib" > "$out"
while true; do
  ...
  echo -e "$(date +%H:%M:%S)\t$(mib "$gtt")\t$(mib "$vram")\t$(mib "$avail")\t${busy}\t${rss}" >> "$out"
  sleep "$interval"
done
